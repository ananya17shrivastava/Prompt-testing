...
def lambda_handler(event, context):
    ...
    for record in event['Records']:
        ...
        prompts=get_kpi_prompt(solution_name,usecase_name,usecase_description,industry_category_name,industry_name,langfuse)
        ...
        description_result = invoke_llm(conn,provider, model, [{
            "role": "user",
            "content": user_prompt,
        }], max_tokens=4096, temperature=.2,prompt_id="kpi_prompt",system_prompt=system_prompt,API_KEY=PERPLEXITY_API_KEY)
        conn.close()
        ...
        result= invoke_llm(conn,provider, model, [{
                "role": "user",
                "content": xml_prompt,
        }], max_tokens=4096, temperature=0,prompt_id="kpi_prompt_xml",API_KEY=ANTHROPIC_API_KEY)
        conn.close()
