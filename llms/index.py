...
def invoke_llm(provider: str, model: str, messages: List[Dict[str, str]], max_tokens: int, temperature: float = 0,prompt_id: str="",system_prompt: str=""):
    if (provider == LLM_PROVIDER_CLAUDE):
        return  call_llm_claude(max_tokens=max_tokens, messages=messages, model=model, temperature=temperature,prompt_id=prompt_id,system_prompt=system_prompt)
    elif (provider == LLM_PROVIDER_GPT):
        return  call_openai(max_tokens=max_tokens, messages=messages, model=model, temperature=temperature,prompt_id=prompt_id,system_prompt=system_prompt)
    elif (provider == LLM_PROVIDER_PERPLEXITY):
        return  call_llm_perplexity(max_tokens=max_tokens, messages=messages, model=model, temperature=temperature,prompt_id=prompt_id,system_prompt=system_prompt)
    else:
